...
class Challenge extends Sequelize.Model {
   ...
   static init(sequelize) {
    return super.init(
      ...
      {  // 두번째 객체 인수는 테이블 자체에 대한 설정
        ...
        modelName: 'Challenge', /* 모델 이름을 설정. */
        tableName: 'challenges', /* 데이터베이스의 테이블 이름. */
   ...
   static associate(db) { // 인자로 index.js에서 만든 여러 테이블이 저장되어있는 db객체를 받을 것이다.
      ...
      db.Challenge.belongsTo(db.User, { foreignKey: "userId", targetKey: "userId" });
      db.Challenge.hasMany(db.ChallengeAttendance, { foreignKey: "challengeId", sourceKey: "challengeId" });
   }
};
